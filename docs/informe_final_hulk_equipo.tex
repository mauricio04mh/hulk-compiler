...
{\small\begin{longtable}{p{3.0cm}p{3.2cm}p{2.8cm}p{5.0cm}}
\toprule
...
Las expresiones del AST cubren todos los constructos del lenguaje: \src{Number}, \src{String}, \src{Bool}, \src{Var}, \src{Unary}, \src{Binary}, \src{Assign}, \src{Let}, \src{Block}, \src{If}, \src{While}, \src{For}, \src{Call}, \src{MethodCall}, \src{New}, \src{MemberAccess}, \src{SelfRef}, \src{BaseCall}, \src{TypeTest}, \src{TypeCast}, \src{VectorLiteral}, \src{VectorGenerator}, \src{NewVector}, \src{VectorIndex}, \src{Lambda}. Cada nodo incluye un \src{Span} con información de posición para diagnósticos.
...
\begin{itemize}
    \item \textbf{hulk-parsegen:} pruebas unitarias del Pratt parser que verifican precedencia, asociatividad, paréntesis, unarios, errores de paréntesis no cerrado y errores de RHS faltante.
    \item \textbf{hulk-sema:} pruebas de chequeo de programas válidos e inválidos; pruebas de inferencia desde cuerpo, desde sitio de llamada y de parámetros de métodos.
    \item \textbf{hulk-codegen-llvm:} pruebas de integración que compilan con \src{clang} y verifican la salida estándar de 35 casos concretos, incluyendo aritmética, booleanos, strings, concatenación, funciones, recursión, ciclos y builtins matemáticos.
\end{itemize}

\subsection{Pruebas de integración ejecutable}

El conjunto \src{SUPPORTED\_CASES} en \src{hulk-codegen-llvm/tests/backend\_minimal.rs} define los casos de prueba que se ejecutan end-to-end: desde el código fuente \hulk{} hasta la comparación de la salida estándar del binario compilado. Esta es la validación más fuerte del compilador, porque verifica que el pipeline completo —semántica, IR, LLVM, runtime— produce el resultado correcto.

Adicionalmente, hay una prueba de comportamiento negativo (\src{unsupported\_vector\_fails\_cleanly}) que verifica que el backend falla con un error \src{LlvmCodegenError::Unsupported} en lugar de generar código incorrecto, cuando se usa una característica que no está completamente implementada.

\subsection{CI}

El workflow de GitHub Actions ejecuta \src{cargo test --workspace --all-targets --locked}, que valida todas las pruebas del workspace bajo una instalación limpia de Rust estable. El workflow también valida el REPORT.md mediante conteo de palabras (mínimo 2000). La ejecución de pruebas \hulk{} se hace mediante \src{make test}, que requiere el binario \src{hulkc} y la carpeta de tests.
